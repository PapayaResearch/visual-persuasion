...
class VisualNudgeCompetition(ABC):
    """
    Abstract base class for visual nudging competitions.
    Defines the interface for running competitions between images.
    """
    @abstractmethod
    def run(self, image_paths: list[str], results_dir: str, max_workers: int):
        pass


@dataclasses.dataclass
class StandardCompetition(VisualNudgeCompetition):
    """
...
@dataclasses.dataclass
class SelectorBasedCompetition(VisualNudgeCompetition):
    """
    Orchestrates paired contests between images, iteratively improving losers until equilibrium.
    Each pair of images competes, and the loser is improved based on judge feedback.
    Process continues until neither image can be improved further (equilibrium).
...
class OptimizationPipeline:
    ...
    # Selector-based competition settings
    use_last_winner_as_base: bool
    always_zero_shot: bool
    editing_context_prompt: str
    background_state_prompt: str
    num_improvement_proposals: int
    proposer_model: LanguageModel
    ...
    def _create_competition(self) -> VisualNudgeCompetition:
        ...
        if self.use_selector:
            competition = SelectorBasedCompetition(
                name=self.name,
                ...
                always_zero_shot=self.always_zero_shot,
