...
def main():
    ...
    dfs = []
    for filename in glob.glob(os.path.join(sys.argv[1], "**/results_pairs.csv"), recursive=True):
        df = pd.read_csv(filename)
        df["source_file"] = filename
        df["model"] = filename.split("/")[-2]

        cfg_path = os.path.join(os.path.dirname(filename), "config_pairs.yaml")
    ...
    df.to_csv(
        os.path.join(sys.argv[1], "combined_results-%s.csv" % datetime.now().strftime("%Y%m%d-%H%M%S")),
        index=False
    )
